...
export function ShapeGrid({
  ...
  squareSize = 40,
  hoverFillColor = 'rgba(255, 166, 61, 0.1)',
  shape = 'square',
  hoverTrailAmount = 3
}: ShapeGridProps) {
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      // We remove the default hardcoded #120F17 gradient from the original so it fits our theme transparently
    };
    ...
    // Initial draw in case reduced motion is true
    drawGrid();
    if (!prefersReduced) {
      requestRef.current = requestAnimationFrame(updateAnimation);
    }
  ...
  }, [direction, speed, borderColor, hoverFillColor, squareSize, shape, hoverTrailAmount, prefersReduced]);
